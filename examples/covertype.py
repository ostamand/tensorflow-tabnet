...
def train(run_name: Text):
    df_tr, df_val, df_test = get_data("data/covtype.csv")
    ...
    optimizer = tf.keras.optimizers.Adam(learning_rate=lr, clipnorm=CONFIGS["clipnorm"])
    ...
    log_dir = (
        LOGDIR + "/" + datetime.strftime(datetime.now(), "%Y-%m-%d-%H-%M-%S")
        if run_name is None
        else LOGDIR + "/" + run_name
    )
