...
def dataset() -> tf.data.Dataset:
    ...
    y = (
        (2 * tf.random.uniform((size_of_dataset, 1), minval=0.9, maxval=1.1)) * x
        + 10
        + tf.random.uniform((size_of_dataset, 1), minval=-2.0, maxval=2.0)
    )
...
def test_can_run_the_lr_finder(model: tf.keras.Model, dataset: tf.data.Dataset):
    ...
    model.compile(
        optimizer=tf.keras.optimizers.SGD(), loss=tf.keras.losses.MeanSquaredError()
    )
